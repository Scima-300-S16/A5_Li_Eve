...
void setup() {
  size(1425, 742);
  mapImage = loadImage("world.png");
  
  Gif = loadImage ("madein.gif");
...
//count through rows of location table, 
  for (int row = 0; row<rowCount; row++) {
    ...
    String definition = locationTable.getRowName(row);
    //get the 2nd and 3rd fields and assign them to
    float x = locationTable.getFloat(id, 2);
    float y = locationTable.getFloat(id, 3);
    float newX = map (x, 167, 1180, 170, 1180);
    float newY = map (y, 133, 391, 90, 450);
...
//if the value variable holds a float greater than or equal to 0
  if (value>=0) {
    ...
    radius = map(value, 0, dataMin, 0, 30);
    noStroke();
    fill(#EC5166); // red
  }
...
//if the mouse is hovering over circle, show information as text
  if ((d<radius+2) && (d<closestDist)) {
    ...
    String definition = locationTable.getString(id, 4);
    text(definition, x+30, y-20, 200, 400); 
    closestText = name +" ";
    closestTextX = x;
